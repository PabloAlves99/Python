def adicionarSerie():
    ...
    while True:
        ...
        if serie.lower() in [s.lower() for s in series]: # Se serie(em minusculo) em [ S.lower recebe a serie dentro do for em minusculo e o "s" recebe oque tem dentro da lista] 
            # Se o serie antes do "in" for igual o s depois do "in"...
            print("Essa série já foi adicionada. Digite outra série ou 'sair' para encerrar.")
        else:
            series.append(serie) # Adicionar na lista
            if len(series) >= max_series:
                print(f"Limite máximo de séries ({max_series}) atingido. Encerrando adição de séries.")
                break
