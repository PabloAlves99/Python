...
def divide(a, b):
    if b != 0:
        ...
        raise ValueError(
            'Não é possível dividir por zero. Por favor, insira um divisor diferente de zero.')
...
def calculate():
    ...
    clear_screen()
    print(
        '\033[1mDigite apenas oque estiver entre [] !!\n\tOperações disponiveis\033[0m')
    ...
    if chosen_operation in operation:
        ...
        clear_screen()
        raise ValueError(
            'Operação inválida. Por favor, escolha uma operação válida listada entre os símbolos [+, -, *, /, %].')
